...
-- 1. Go to: https://supabase.com/dashboard/project/hbtaablzueprzuwbapyj/sql/new
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'encrypted_secrets') THEN
        RAISE NOTICE 'Migration not yet applied. Proceeding...';
    ELSE
        RAISE NOTICE 'Migration may already be applied. Check existing tables.';
    END IF;
END $$;
...
-- Enable pgcrypto for encryption (skip if exists)
...
AND tablename IN ('encrypted_secrets', 'audit_logs')
ORDER BY tablename;

-- Check created functions:
SELECT routine_name FROM information_schema.routines
WHERE routine_schema = 'public'
AND routine_name IN ('anonymize_user_data', 'export_user_data', 'store_encrypted_secret', 'get_decrypted_secret', 'log_audit_event')
...
-- ✅ Data Retention: cleanup_old_analytics(), cleanup_old_social_shares()
-- ✅ Encryption: encrypted_secrets table with pgcrypto
-- ✅ Audit Logging: audit_logs table with automatic triggers