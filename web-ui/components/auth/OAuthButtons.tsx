...
const OAuthButtons = memo(function OAuthButtons({
  ...
  const handleOAuthLogin = async (providerName: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/auth/oauth/${providerName}/login`);
      if (!response.ok) {
  ...
  const getProviderIcon = (provider: string) => {
    switch (provider.toLowerCase()) {
      ...
      default:
        return (
          <div className="w-5 h-5 bg-gray-600 rounded-full flex items-center justify-center text-xs font-bold">
      ...
      <div className="grid grid-cols-1 gap-3">
        {providers.map((provider) => (
          <button
            ...
            className={`w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-700 rounded-lg hover:bg-gray-800 transition disabled:opacity-50 disabled:cursor-not-allowed ${
              authenticating === provider.name ? 'bg-gray-800' : ''
            }`}
          >
